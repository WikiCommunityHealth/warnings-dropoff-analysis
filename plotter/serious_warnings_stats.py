...
def plot_warnings_stats(file_path: str) -> None:
    ...
    plot_pie_chart(
        ...
        data=[
            df.loc[df['average_edit_amount_after_last_serious_warning_date'] == 0].shape[0], 
            df.loc[df['average_edit_amount_after_last_serious_warning_date'] > 0].shape[0]], 
        labels=['Has stopped editing', 'Has continued editing']
    )
    # decrease activity
    df['Decrease activity'] = df['average_edit_amount_after_last_serious_warning_date'] < df['average_edit_amount_before_last_serious_warning_date']
    print('Plotting chart n {}...'.format(chart_counter))
    chart_counter += 1
    plot_pie_chart(
    ...
    try:
        user = df[
                ...
                df['amount_serious_templates_transcluded'] == df['amount_serious_templates_transcluded'].max()
            ].iloc[0]
    except:
    ...
    plot_line_chart(
        ...
        title = 'User analysis: warnings recieved compared to the activity',
    ...
    plot_line_chart(
        ...
        title = 'User analysis: warnings recieved',
